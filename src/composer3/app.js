...
(() => {
  ...
  function bindInterface() {
    ...
    $('#staffPages').addEventListener('click', event => {
      ...
      if (!target) return;
      lastSelectionAnchorId = String(target.dataset.eventId || '');
      engine.selectEvent(lastSelectionAnchorId, { additive: event.ctrlKey || event.metaKey, toggle: event.ctrlKey || event.metaKey });
    });
    ...
    $('#staffPages').addEventListener('keydown', event => {
      ...
      if (event.key === 'Enter' || event.key === ' ') {
        event.preventDefault();
        lastSelectionAnchorId = String(target.dataset.eventId || '');
        engine.selectEvent(lastSelectionAnchorId, { additive: event.ctrlKey || event.metaKey });
      }
    });
