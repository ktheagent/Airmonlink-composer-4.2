...
test('Build 40 piano workflow preserves chords, tuplets, marks, pedal and layout through reopen', () => {
  ...
  const chord = engine.addPianoChord([60, 64, 67], { start: 0, duration: 1, staff: 'treble' });
  engine.selectEvents(chord.map(event => event.id));
  ...
  assert.ok(before.every(event => event.tuplet?.actual === 3));
  ...
  assert.deepEqual(after.map(event => [event.id, event.midi, event.start, event.duration]), before.map(event => [event.id, event.midi, event.start, event.duration]));
...
test('Build 40 metadata and workflow names identify the same release', () => {
  const packageJson = require('../package.json');
  const main = fs.readFileSync(path.join(__dirname, '../src/composer3/main.js'), 'utf8');
  const html = fs.readFileSync(path.join(__dirname, '../src/composer3/index.html'), 'utf8');
  assert.equal(packageJson.version, '1.2.0');
  assert.equal(Number(packageJson.buildNumber), 40);
  assert.equal(packageJson.buildVersion, '1.2.0.40');
  assert.match(packageJson.build.nsis.artifactName, /Build40-Setup/);
  assert.match(packageJson.build.portable.artifactName, /Build40-Portable/);
  assert.match(main, /const BUILD = 40/);
  assert.match(html, /1\.2\.0 · Build 40/);
});
