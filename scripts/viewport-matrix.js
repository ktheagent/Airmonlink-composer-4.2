...
async function runScenario(scenario, index, previewHtml) {
  ...
  const profile = fs.mkdtempSync(path.join(os.tmpdir(), `airmon-build30-matrix-${index}-`));
  ...
  try {
    ...
    const screenshotName = `build30-viewport-${scenario.width}x${scenario.height}-${String(scenario.scale).replace('.', '_')}x.png`;
...
async function main() {
  ...
  fs.writeFileSync(path.join(validation, 'build30-viewport-matrix.json'), JSON.stringify(report, null, 2) + '\n');
